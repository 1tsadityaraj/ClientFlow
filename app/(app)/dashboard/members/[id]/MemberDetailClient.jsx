  ...
  return (
    <main className="min-h-screen bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-900 dark:text-zinc-50">
      <div className="mx-auto max-w-5xl px-6 py-10">
        ...
        {/* Profile header */}
        <div className="mt-6 rounded-2xl border border-zinc-200 dark:border-zinc-800/80 bg-zinc-50/80 dark:bg-zinc-100 dark:bg-zinc-100 dark:bg-zinc-900/40 p-6">
          <div className="flex items-start gap-5">
            ...
            <div className="flex-1">
              ...
              {member.status?.currentWork && (
                <p className="mt-1 text-sm italic text-zinc-600 dark:text-zinc-600 dark:text-zinc-400">
                  "{member.status.currentWork}"
                </p>
              )}
        ...
        <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <StatBox label="Todo" value={taskStats.todo} icon={<AlertCircle className="h-4 w-4 text-zinc-600 dark:text-zinc-600 dark:text-zinc-400" />} />
        </div>

        {/* Completion bar */}
        <div className="mt-4 rounded-2xl border border-zinc-200 dark:border-zinc-800/80 bg-zinc-50/80 dark:bg-zinc-100 dark:bg-zinc-100 dark:bg-zinc-900/40 p-5">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-medium text-zinc-600 dark:text-zinc-600 dark:text-zinc-400">Overall completion</span>
            <span className="text-xs font-semibold text-zinc-800 dark:text-zinc-200">{taskStats.completionRate}%</span>
          </div>
        ...
        {activeTasks.length > 0 && (
          <section className="mt-6">
            ...
            <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800/80 bg-zinc-50/80 dark:bg-zinc-100 dark:bg-zinc-100 dark:bg-zinc-900/40 overflow-hidden">
              {activeTasks.map((task, i) => (
                <Link
                  key={task.id}
                  href={`/dashboard/projects/${task.projectId}`}
                  className={`flex items-center gap-4 px-5 py-3.5 transition-colors hover:bg-zinc-200 dark:bg-zinc-800/40 ${
                    i < activeTasks.length - 1 ? "border-b border-zinc-200 dark:border-zinc-800/50" : ""
                  }`}
                >
                  <span
            ...
            <div className="space-y-2">
              {projects.map((p) => (
                <Link
                  ...
                  className="flex items-center gap-4 rounded-xl border border-zinc-200 dark:border-zinc-800/80 bg-zinc-50/80 dark:bg-zinc-100 dark:bg-zinc-100 dark:bg-zinc-900/40 px-5 py-4 transition-all hover:border-zinc-300 dark:border-zinc-300 dark:border-zinc-700 hover:bg-zinc-50/90 dark:bg-zinc-100 dark:bg-zinc-100 dark:bg-zinc-900/60"
                >
                  <span
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-1.5 flex h-1.5 overflow-hidden rounded-full bg-zinc-200 dark:bg-zinc-800 max-w-xs">
                      <div
                  ...
                  <span className="rounded-full bg-zinc-200 dark:bg-zinc-800 px-2 py-0.5 text-[9px] font-medium uppercase tracking-wide text-zinc-600 dark:text-zinc-600 dark:text-zinc-400">
                    {p.role}
                  </span>
                </Link>
              ))}
...
function StatBox({ label, value, icon }) {
  return (
    <div className="rounded-xl border border-zinc-200 dark:border-zinc-800/80 bg-zinc-50/80 dark:bg-zinc-100 dark:bg-zinc-100 dark:bg-zinc-900/40 p-4">
      <div className="flex items-center justify-between mb-2">
      ...
      <p className="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-900 dark:text-zinc-50">{value}</p>
    </div>
  );
}
